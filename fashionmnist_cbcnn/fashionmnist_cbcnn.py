"""Train and evaluate a binary CNN on Fashion-MNIST."""
...
def train_model(epochs=500, batch_size=50):
    tf.keras.utils.set_random_seed(SEED)
    (x_train, y_train), (x_test, y_test) = load_data()
    model = build_model()

    lr_decay = (LR_END / LR_START) ** (1.0 / max(epochs, 1))
    checkpoint_path = OUTPUT_DIR / "fashion_mnist_cbcnn.best.weights.h5"
    callbacks = [
    ...
    model.save(OUTPUT_DIR / "fashion_mnist_cbcnn.keras")
